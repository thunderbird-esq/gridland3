...
def run_scan(job_id: str, target: str, aggressive: bool = False, threads: int = 100) -> None:
    """
    ...
    try:
        ...
        if '/' in target:
            ...
            try:
                ...
                add_job_log(job_id, f"Scanning network range: {target}")
                
                for ip_obj in network.hosts():
                    ip = str(ip_obj)
                    add_job_log(job_id, f"Scanning {ip}...")
                    
                    result = _scan_single_target(job_id, ip, aggressive, threads)
                    if result:
            ...
            result = _scan_single_target(job_id, target, aggressive, threads)
            if result:
                results.append(result)
                add_job_log(job_id, f"Found device at {target}: {result.device_type or 'Unknown'} ({len(result.open_ports)} ports)")
...
def _scan_single_target(job_id: str, ip: str, aggressive: bool, threads: int) -> ScanTarget:
    ...
    # Step 1: Port scan
    add_job_log(job_id, f"Port scanning {ip}...")
