...
class StreamDiscoveryPlugin:
    ...
    def load_stream_paths_database(self):
        ...
        try:
            db_path = Path(__file__).parent.parent.parent / 'gridland' / 'data' / 'stream_paths.json'
            if db_path.exists():
...
class TestStreamDiscoveryConfiguration(unittest.TestCase):
    ...
    def test_results_lock_initialized(self):
        ...
        plugin = StreamDiscoveryPlugin()
        self.assertIsInstance(plugin.results_lock, threading.Lock)
...
class TestStreamDiscoveryRTSP(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.mock_detector.check_stream_url = AsyncMock(return_value={
            'is_stream': True,
            'detection_method': 'protocol_detection'
        })
        ...
        self.mock_rtsp_handler.build_url = Mock(side_effect=lambda ip, port, path: f'rtsp://{ip}:{port}{path}')
    ...
    async def test_rtsp_multiple_ports(self):
        ...
        streams = await self.plugin.discover_streams('192.168.1.100', [554, 8554], protocols=['rtsp'])
...
class TestStreamDiscoveryRTMP(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.mock_rtmp_handler.build_url = Mock(side_effect=lambda ip, port, path: f'rtmp://{ip}:{port}{path}')
...
class TestStreamDiscoveryHTTP(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.mock_http_handler.build_url = Mock(side_effect=lambda ip, port, path: f'http://{ip}:{port}{path}')
...
class TestStreamDiscoveryIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_integration_with_protocol_handlers(self):
        ...
        try:
            from test_protocol_handlers import RTSPHandler, HTTPHandler
