...
class StreamDetector:
    ...
    def __init__(self):
        self.video_content_types = [
            'video/mp4', 'video/h264', 'video/h265', 'video/hevc',
            'video/mpeg', 'video/x-msvideo', 'video/quicktime',
            'video/webm', 'video/ogg', 'video/x-flv',
            'image/jpeg', 'image/png',  # For MJPEG streams
        ...
        self.stream_url_patterns = [
            '.mp4', '.m3u8', '.ts', '.mpd', '.mjpg', '.mjpeg',
            '/video', '/stream', '/live', '/snapshot', '/cam',
            '/media', '/feed', '/channel'
        ]
...
class TestStreamDetector(unittest.TestCase):
    ...
    def test_categorize_stream_types(self):
        ...
        self.assertEqual(
            self.detector.categorize_stream('http://example.com/snapshot.jpg', 'image/jpeg', 'http'),
            'snapshot'
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.detector.categorize_stream('http://example.com/stream.m3u8', 'application/vnd.apple.mpegurl', 'http'),
            'hls'
        )
        self.assertEqual(
...
class TestStreamDetectorAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_request_with_content_analysis(self, mock_session):
        ...
        result = await self.detector.check_stream_url('http://example.com/stream.unknown', method='GET')
    ...
    async def test_url_pattern_fallback(self, mock_session):
        ...
        result = await self.detector.check_stream_url('http://example.com/video/stream.mp4', method='HEAD')
