...
class TestDiscoverCameraPorts:
    ...
    def test_camera_port_category_choices(self, runner):
        ...
        assert 'rtsp' in result.output.lower() or 'web' in result.output.lower() or 'all' in result.output.lower()
...
class TestDiscoverInputValidation:
    ...
    def test_masscan_requires_range(self, runner):
        ...
        result = runner.invoke(discover, [
            '--engine', 'masscan',
            '--query', 'camera'  # Wrong input type for masscan
        ])
        # Should fail or warn
        assert result.exit_code != 0 or 'range' in result.output.lower() or 'input' in result.output.lower()
