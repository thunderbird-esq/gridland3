...
class DahuaScanner(VulnerabilityPlugin):
    ...
    async def _test_rpc2_login(self, base_url: str, username: str, password: str) -> bool:
        ...
        try:
            ...
            async with self.session.post(login_url, json=challenge_data) as response:
                ...
                pass_hash = (
                    hashlib.md5(f"{username}:{realm}:{password}".encode()).hexdigest().upper()
                )
                response_hash = (
                    hashlib.md5(f"{username}:{challenge}:{pass_hash}".encode()).hexdigest().upper()
                )
            ...
            async with self.session.get(cgi_url, auth=auth) as response:
                if response.status == 200:
                    ...
                    return any(
                        ...
                        for indicator in ["devicetype", "serialnumber", "version", "builddate"]
                    )
    ...
    def _parse_system_info(self, content: str, ip: str, port: int, path: str) -> Any | None:
