...
class AxisScanner(VulnerabilityPlugin):
            ...
            async with self.session.get(admin_url, auth=auth) as response:
                if response.status == 200:
                    ...
                    return any(
                        ...
                        for indicator in ["root.System", "System=", "SerialNumber", "HardwareID"]
                    )
    ...
    async def _test_vapix_vulnerabilities(
        self, base_url: str, target_ip: str, target_port: int
    ) -> list[Any]:
            ...
            try:
                ...
                if cve_data["method"] == "GET":
                    ...
                    async with self.session.post(
                        test_url, data=test_data, headers=headers
                    ) as response:
                        vulnerable = await self._check_cve_response(response, cve_id)
                else:
                    continue
        ...
        try:
            ...
            async with self.session.get(test_url) as response:
                if response.status == 200:
                    ...
                    fingerprint_vuln = self._parse_vapix_properties(
                        content, target_ip, target_port, fingerprint_path
                    )
                    if fingerprint_vuln:
    ...
    def _parse_vapix_properties(self, content: str, ip: str, port: int, path: str) -> Any | None:
