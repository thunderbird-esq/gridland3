...
class BannerGrabber(VulnerabilityPlugin):
    ...
    def __init__(self):
        ...
        self.camera_patterns = {
            ...
            "sony": [r"Sony.*Network.*Camera", r"SNC-.*", r"Sony.*Corporation"],
            "bosch": [r"Bosch.*Security", r"NBC-.*", r"VIP-.*", r"Bosch.*Camera"],
            "panasonic": [r"Panasonic.*Network.*Camera", r"WV-.*", r"Panasonic.*Corporation"],
            "vivotek": [r"VIVOTEK.*Inc", r"IP.*Camera", r"Vivotek.*HTTP"],
            "foscam": [r"Foscam.*Camera", r"FI.*Series", r"Foscam.*HTTP"],
            "generic_camera": [
                r"IP.*Camera",
                r"Network.*Camera",
                r"CCTV",
    ...
    async def scan_vulnerabilities(
        ...
        try:
            ...
            logger.info(
                f"🔬 Fingerprint result: {fingerprint_signature.brand} "
    ...
    def _get_service_probe(self, target_ip: str, target_port: int) -> str | None:
    ...
    def _detect_camera_brand(self, banner: str) -> str | None:
    ...
    def _check_version_vulnerabilities(
        self, banner: str, target_ip: str, target_port: int
    ) -> list[Any]:
        ...
        vulnerable_versions = {
            ...
            "boa": {r"Boa/0\.[0-9]\.[0-9]": "Boa web server has multiple known vulnerabilities"},
        }
    ...
    async def _analyze_http_service(
        ...
        try:
            ...
            async with self.session.get(url) as response:
                ...
                if info_headers:
                    ...
                    vuln.description = (
                        f"Information disclosure in headers: {', '.join(info_headers)}"
                    )
                    vuln.exploit_available = False
                    results.append(vuln)
    ...
    async def _get_ssl_certificate(self, target_ip: str, target_port: int) -> dict[str, Any] | None:
        """Get SSL certificate information."""
        try:
            # Create SSL context
    ...
    def _check_ssl_vulnerabilities(
        self, cert_info: dict[str, Any], target_ip: str, target_port: int
    ) -> list[Any]:
    ...
    async def _analyze_service_with_fingerprint(
        self, banner: str, target_ip: str, target_port: int, fingerprint_signature
    ) -> list[Any]:
        """
    ...
    async def _create_fingerprint_vulnerability_results(
        self, target_ip: str, target_port: int, fingerprint_signature
    ) -> list[Any]:
        """
        ...
        try:
            ...
            for cve in fingerprint_signature.vulnerability_indicators:
                ...
                cve_result.description = (
                    f"Potential {fingerprint_signature.brand} vulnerability {cve}"
                )
    ...
    async def _analyze_http_service_enhanced(
        self, target_ip: str, target_port: int, service: str, fingerprint_signature
    ) -> list[Any]:
        """
    ...
    async def _analyze_ssl_service_enhanced(
        self, target_ip: str, target_port: int, fingerprint_signature
    ) -> list[Any]:
        """
        ...
        try:
            ...
            if "connection_behaviors" in behavioral_metrics:
                ...
                if isinstance(connection_patterns, list) and len(connection_patterns) >= 3:
                    success_rate = sum(1 for x in connection_patterns if x) / len(
                        connection_patterns
                    )
    ...
    async def _check_brand_specific_http_vulns(
        self, target_ip: str, target_port: int, fingerprint_signature
    ) -> list[Any]:
        """
