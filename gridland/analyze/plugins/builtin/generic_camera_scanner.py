...
class GenericCameraScanner(VulnerabilityPlugin):
    ...
    def __init__(self):
        ...
        self.common_paths = {
            ...
            "api": ["/api/", "/cgi-bin/", "/onvif-http/snapshot", "/img/main.cgi"],
        }
    ...
    async def scan_vulnerabilities(
        ...
        try:
            ...
            unprotected_path_vulns = await self._test_unprotected_paths(
                base_url, target_ip, target_port
            )
            ...
            async with self.session.get(base_url) as response:
                ...
                if "text/html" in content_type:
                    ...
                    if title_match:
                        ...
                        if any(
                            kw in title for kw in ["camera", "webcam", "dvr", "nvr", "surveillance"]
                        ):
                            return True, content
    ...
    async def _test_default_credentials(
        self, base_url: str, target_ip: str, target_port: int
    ) -> list[Any]:
        ...
        for endpoint in login_endpoints:
            for username, password in self.default_credentials:
                try:
                    ...
                    basic_success = await self._test_basic_auth(
                        urljoin(base_url, endpoint), username, password
                    )
                    if basic_success:
                    ...
                    form_success = await self._test_form_auth(
                        urljoin(base_url, endpoint), username, password
                    )
                    if form_success:
                        vuln = self.memory_pool.acquire_vulnerability_result()
                        ...
                        vuln.confidence = 0.90
                        vuln.description = (
                            f"Form-based default credentials: {username}/{password} at {endpoint}"
                        )
                        vuln.exploit_available = True
                        results.append(vuln)
                        return results
        ...
        try:
            ...
            async with self.session.get(url, auth=auth) as response:
                if response.status == 200:
                    ...
                    return any(
                        indicator in content.lower() for indicator in self.success_indicators
                    )
                        ...
                        async with self.session.post(form_action, data=fields) as login_response:
                            if login_response.status in [200, 302]:
                                ...
                                if login_response.status == 302:
                                    ...
                                    if any(
                                        indicator in login_content.lower()
    ...
    def _extract_form_action(self, html_content: str, base_url: str) -> str | None:
                ...
                async with self.session.get(test_url) as response:
                    if response.status == 200:
                        ...
                        if any(indicator in content for indicator in unix_indicators):
                            ...
                            vuln.description = (
                                f"Directory traversal vulnerability: {traversal_path}"
                            )
                            vuln.exploit_available = True
                            results.append(vuln)
                            break
    ...
    async def _test_weak_configurations(
        self, base_url: str, target_ip: str, target_port: int
    ) -> list[Any]:
    ...
    async def _test_unprotected_paths(
        self, base_url: str, target_ip: str, target_port: int
    ) -> list[Any]:
