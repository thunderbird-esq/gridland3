...
class HikvisionScanner(VulnerabilityPlugin):
            ...
            try:
                ...
                async with self.session.get(test_url) as response:
                    if response.status == 200:
                        ...
                        if any(
                            pattern.lower() in content.lower() for pattern in sensitive_patterns
                        ):
                            vuln = self.memory_pool.acquire_vulnerability_result()
                            vuln.ip = target_ip
                            vuln.port = target_port
    ...
    def _parse_device_info_xml(self, xml_content: str, ip: str, port: int, path: str) -> Any | None:
    ...
    def _parse_config_file_xml(self, xml_content: str, ip: str, port: int, path: str) -> Any | None:
