...
class StreamPathOptimizer:
    ...
    def __init__(self, stream_database: dict):
        ...
        self.brand_indicators = stream_database.get("optimization", {}).get(
            "brand_priority_indicators", {}
        )

    def optimize_path_order(self, paths: list[str], brand: str | None, protocol: str) -> list[str]:
        ...
        def path_score(path: str) -> float:
            ...
            high_success = self.stream_database.get("optimization", {}).get(
                "high_success_paths", []
            )
...
class EnhancedStreamScanner(VulnerabilityPlugin):
    ...
    async def scan_vulnerabilities(
        ...
        try:
            ...
            logger.info(
                f"✅ Enhanced scanning complete: {len(vulnerability_results)} findings in {scan_time:.2f}s"
            )
    ...
    async def _detect_target_brand(
        self, target_ip: str, target_port: int, service: str, banner: str
    ) -> dict[str, any]:
    ...
    def _detect_brand_from_banner(self, banner: str) -> str | None:
    ...
    async def _detect_brand_from_http_headers(
    ...
    async def _detect_brand_from_behavior(
        self, target_ip: str, target_port: int, service: str
    ) -> str | None:
    ...
    async def _test_rtsp_endpoint(
        self, target_ip: str, target_port: int, path: str
    ...
    def _calculate_rtsp_confidence(self, path: str, brand: str | None, stream_info: dict) -> float:
    ...
    def _calculate_http_confidence(
        self, content_type: str, path: str, brand: str | None, headers: dict
    ) -> float:
    ...
    def _estimate_http_quality(
        self, content_type: str, content_length: str, headers: dict
    ) -> float:
    ...
    async def _create_vulnerability_reports(
        self, target_ip: str, target_port: int, streams: list[StreamEndpoint], brand_info: dict
    ) -> list[any]:
        ...
        for stream in streams:
            try:
                ...
                vuln_result.description = (
                    f"Enhanced {stream.protocol.upper()} stream discovered: {stream.url}"
                )
        ...
        if len(streams) > 1:
            ...
            summary_result.description = (
                f"Multiple stream endpoints discovered: {len(streams)} streams across protocols"
            )
            summary_result.exploit_available = any(not s.authentication_required for s in streams)

            summary_details = {
                "total_streams": len(streams),
                "protocols": list({s.protocol for s in streams}),
                "unauthenticated_streams": len(
                    [s for s in streams if not s.authentication_required]
                ),
                "high_quality_streams": len([s for s in streams if s.quality_score >= 0.8]),
                "brands_detected": list({s.brand for s in streams if s.brand}),
    ...
    def _update_scan_statistics(self, streams: list[StreamEndpoint]):
        ...
        if self.scan_stats["total_endpoints_tested"] > 0:
            success_rate = (
                self.scan_stats["successful_discoveries"]
                / self.scan_stats["total_endpoints_tested"]
            )
        ...
        self.scan_stats["average_response_time"] = total_time / max(
            1, self.scan_stats["total_endpoints_tested"]
        )
