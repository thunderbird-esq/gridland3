...
from gridland.analyze.plugins.manager import PluginMetadata, StreamPlugin
...
class RTSPStreamScanner(StreamPlugin):
    ...
    async def _scan_rtsp_over_http(
        ...
        for path in http_stream_paths:
            try:
                ...
                for username, password in self.default_credentials:
                    auth_accessible = await self._test_http_stream_auth(
        ...
        for username, password in self.default_credentials:
            for path in self.stream_paths:
                try:
                    ...
                    logger.debug(
                        f"Authenticated RTSP test error for {username}:{password}@{path}: {e}"
                    )
                    continue
    ...
    async def _test_rtsp_connection(self, stream_url: str) -> tuple[bool, dict[str, Any] | None]:
    ...
    async def _test_rtsp_buffer_overflow(self, target_ip: str, target_port: int) -> Any | None:
    ...
    async def _test_rtsp_auth_bypass(self, target_ip: str, target_port: int) -> Any | None:
    ...
    async def _test_rtsp_info_disclosure(self, target_ip: str, target_port: int) -> Any | None:
        ...
        try:
            ...
            try:
                ...
                info_request = (
                    f"GET_PARAMETER rtsp://{target_ip}:{target_port}/ RTSP/1.0\r\nCSeq: 1\r\n\r\n"
                )
