...
from ..memory import AnalysisResult, get_memory_pool
...
class AdaptiveTaskScheduler:
    ...
    def __init__(
        ...
        self._load_monitor_thread: threading.Thread | None = None
    ...
    def start(self):
        ...
        # Start load monitor
        self._load_monitor_thread = threading.Thread(target=self._load_monitor_loop, daemon=True)
    ...
    def _steal_task(self, thief_worker_id: int) -> Any | None:
    ...
    def _execute_task(
        self, task: tuple[Callable, tuple, dict], worker_id: int, start_time: float
    ) -> TaskMetrics:
    ...
    def _record_failed_task(self, task: Any, worker_id: int, start_time: float, error: str):
    ...
    def _adapt_worker_count(self):
        ...
        with self._stats_lock:
            ...
            if total_pending > self._active_workers * 2 and self._active_workers < self.max_workers:
                ...
                logger.info(
                    f"Scaled up to {self._active_workers} workers (pending: {total_pending})"
                )
                ...
                logger.debug(
                    f"Workers underutilized (pending: {total_pending}, active: {self._active_workers})"
                )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        with self._stats_lock:
            ...
            stats = {
                ...
                "pending_tasks": sum(q.size() for q in self._worker_queues)
                + self._global_task_queue.qsize(),
                "worker_stats": [
                    {
                ...
                if successful_tasks:
                    stats["recent_performance"] = {
                        "average_execution_time": statistics.mean(
                            m.execution_time for m in successful_tasks
                        ),
                        "median_execution_time": statistics.median(
                            m.execution_time for m in successful_tasks
                        ),
                        "success_rate": len(successful_tasks) / len(recent_metrics),
                        "tasks_per_second": len(recent_metrics)
                        / (time.time() - recent_metrics[0].start_time),
                    }
...
_global_scheduler: AdaptiveTaskScheduler | None = None
...
def initialize_scheduler(
    max_workers: int | None = None, steal_ratio: float = 0.5, adaptation_interval: float = 5.0
    ...
    _global_scheduler = AdaptiveTaskScheduler(
        max_workers=max_workers, steal_ratio=steal_ratio, adaptation_interval=adaptation_interval
    )
    _global_scheduler.start()
    return _global_scheduler
