...
class NetworkFingerprintEngine:
    ...
    def _calculate_fingerprint_score(self, node: NetworkNode, signature: dict) -> float:
        ...
        if response_data and signature.get("response_patterns"):
            pattern_matches = sum(
                1 for pattern in signature["response_patterns"] if pattern in response_data
            )
...
class NetworkClusterAnalyzer:
        ...
        for subnet, subnet_nodes in subnet_groups.items():
            if len(subnet_nodes) >= 2:  # At least 2 nodes to form cluster
                cluster = NetworkCluster(
                    ...
                    lateral_movement_potential=self._calculate_lateral_movement_potential(
                        subnet_nodes
                    ),
                )
        ...
        for port_pattern, pattern_nodes in port_signatures.items():
            if len(pattern_nodes) >= 2:
                ...
                cluster = NetworkCluster(
                    ...
                    lateral_movement_potential=self._calculate_port_pattern_lateral_potential(
                        port_pattern
                    ),
                )
                clusters.append(cluster)
            ...
            if len(device_nodes) >= 2:
                ...
                cluster = NetworkCluster(
                    ...
                    characteristics={
                        ...
                        "fingerprint_consistency": self._calculate_fingerprint_consistency(
                            device_nodes
                        ),
                    },
                    vulnerability_score=self._calculate_device_type_vulnerability(device_type),
                    lateral_movement_potential=self._calculate_device_type_lateral_potential(
                        device_type
                    ),
                )
                clusters.append(cluster)
    ...
    def _calculate_subnet_vulnerability(self, nodes: list[NetworkNode]) -> float:
        ...
        camera_nodes = sum(
            1
            for node in nodes
            if any(indicator in str(node.metadata).lower() for indicator in camera_indicators)
        )
    ...
    def _calculate_lateral_movement_potential(self, nodes: list[NetworkNode]) -> float:
        ...
        admin_nodes = sum(
            1 for node in nodes if any(port in node.open_ports for port in admin_ports)
        )
        if admin_nodes > 0:
            potential += 0.3
    ...
    def _calculate_fingerprint_consistency(
        self, device_nodes: list[tuple[NetworkNode, float]]
    ) -> float:
    ...
    def _merge_overlapping_clusters(self, clusters: list[NetworkCluster]) -> list[NetworkCluster]:
        ...
        for i, cluster1 in enumerate(clusters):
            ...
            for j, cluster2 in enumerate(clusters[i + 1 :], i + 1):
                ...
                # Calculate overlap
                nodes1 = {node.ip for node in cluster1.nodes}
                nodes2 = {node.ip for node in cluster2.nodes}
    ...
    def _filter_significant_clusters(self, clusters: list[NetworkCluster]) -> list[NetworkCluster]:
        ...
        for cluster in clusters:
            ...
            if len(cluster.nodes) >= 2 and (  # Minimum size
                cluster.vulnerability_score >= 0.4  # High vulnerability
                or cluster.lateral_movement_potential >= 0.5  # High lateral potential
                or len(cluster.nodes) >= 5
            ):  # Large cluster
        ...
        def significance_score(cluster):
            return (
                cluster.vulnerability_score * 0.4
...
class TopologyDiscoveryEngine:
    ...
    async def discover_network_topology(
        self, target_ip: str, scope: str = "subnet"
    ) -> NetworkTopology:
        """
        ...
        try:
            ...
            logger.info(
                f"✅ Topology discovery complete: {len(nodes)} nodes, {len(clusters)} clusters"
            )
            return topology
    ...
    async def _discover_single_node(
        self, host_ip: str, semaphore: asyncio.Semaphore
    ) -> NetworkNode | None:
        ...
        async with semaphore:
            try:
                ...
                if node.open_ports:
                    node.services = await self._detect_services(
                        host_ip, node.open_ports[:5]
            ...
            for port in common_ports:
                try:
                    _, writer = await asyncio.wait_for(
                        asyncio.open_connection(host_ip, port), timeout=1.0
                    )
                    writer.close()
    ...
    async def _scan_ports(self, host_ip: str, ports: list[int]) -> list[int]:
        ...
        async def scan_port(port):
            try:
                _, writer = await asyncio.wait_for(
                    asyncio.open_connection(host_ip, port), timeout=2.0
                )
                writer.close()
                await writer.wait_closed()
                return port
    ...
    async def _identify_service(self, host_ip: str, port: int) -> str | None:
            ...
            if port == 80:
                ...
                try:
                    reader, writer = await asyncio.wait_for(
                        asyncio.open_connection(host_ip, port), timeout=3.0
                    )
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                async with session.get(f"http://{host_ip}:{port}") as response:
                    ...
                    if any(
                        indicator in server_header + content for indicator in ["hikvision", "webs"]
                    ):
                        return "hikvision-http"
                    elif any(
                        indicator in server_header + content for indicator in ["dahua", "dh-"]
                    ):
                        return "dahua-http"
                    elif "axis" in server_header + content:
                        return "axis-http"
                    elif any(
                        indicator in server_header + content
    ...
    async def _measure_response_times(self, host_ip: str) -> list[float]:
        ...
        for _ in range(3):
            try:
                ...
                try:
                    _, writer = await asyncio.wait_for(
                        asyncio.open_connection(host_ip, 80), timeout=2.0
                    )
                    ...
                    try:
                        start_time = time.time()
                        _, writer = await asyncio.wait_for(
                            asyncio.open_connection(host_ip, 22), timeout=2.0
                        )
                        response_time = (time.time() - start_time) * 1000
                        times.append(response_time)
    ...
    async def _resolve_hostname(self, host_ip: str) -> str | None:
        """Resolve hostname for IP address"""
        try:
            hostname = await asyncio.wait_for(
                asyncio.get_event_loop().run_in_executor(None, socket.gethostbyaddr, host_ip),
                timeout=2.0,
            )
    ...
    async def _detect_security_boundaries(
        self, nodes: list[NetworkNode], clusters: list[NetworkCluster]
    ) -> list[dict[str, any]]:
    ...
    async def _analyze_vulnerability_paths(
        self, nodes: list[NetworkNode], clusters: list[NetworkCluster]
    ) -> list[dict[str, any]]:
        ...
        try:
            ...
            camera_nodes = [
                node for node in nodes if node.device_type and "camera" in node.device_type.lower()
            ]
    ...
    def _suggest_attack_strategy(self, cluster: NetworkCluster) -> list[str]:
        ...
        if cluster.cluster_type == "camera_cluster":
            ...
            strategies.extend(
                ["load_balancer_bypass", "session_prediction", "backend_server_identification"]
            )
        else:
            strategies.extend(
                ["service_enumeration", "vulnerability_scanning", "credential_brute_force"]
            )
    ...
    def _classify_network_devices(self, nodes: list[NetworkNode]) -> list[NetworkNode]:
        ...
        for node in nodes:
            ...
            is_network_device = (
                node.device_type in ["cisco_switch", "generic_router"]
                or (22 in node.open_ports and 23 in node.open_ports and 80 in node.open_ports)
                or (161 in node.open_ports)  # SNMP
            )
