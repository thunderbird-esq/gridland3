...
class AnalysisMemoryPool:
    ...
    def __init__(self, pool_sizes: dict[str, int] | None = None):
        ...
        self.vulnerability_pool = ObjectPool(
            factory=VulnerabilityResult, max_size=self.pool_sizes["vulnerabilities"]
        )

        self.stream_pool = ObjectPool(factory=StreamResult, max_size=self.pool_sizes["streams"])

        self.analysis_pool = ObjectPool(
            factory=AnalysisResult, max_size=self.pool_sizes["analysis_results"]
        )
        ...
        logger.info(
            f"AnalysisMemoryPool initialized with {sum(self.pool_sizes.values())} pre-allocated objects"
        )
...
_global_memory_pool: AnalysisMemoryPool | None = None
...
def initialize_memory_pool(pool_sizes: dict[str, int] | None = None):
