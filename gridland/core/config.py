...
class CameraPortManager:
    ...
    def _load_ports(self):
        """Load ports from the camera_ports.json file."""
        try:
            db_path = Path(__file__).parent.parent / "data" / "camera_ports.json"
            with open(db_path, 'r') as f:
                data = json.load(f)
                self.category_map = data.get("port_categories", {})
                self._compile_ports()
        except Exception as e:
            logger.warning(f"Failed to load camera ports database: {e}, using defaults")
            self._use_default_ports()

    def _compile_ports(self):
        """Compile all ports and priority ports from categories."""
        all_ports = set()
        for category, port_list in self.category_map.items():
            if isinstance(port_list, list):
                all_ports.update(port_list)
            elif isinstance(port_list, dict):
                for sub_category, sub_list in port_list.items():
                    all_ports.update(sub_list)
        self.all_ports = sorted(list(all_ports))
    ...
    def _use_default_ports(self):
        """Use a small set of default ports if the JSON file fails to load."""
        self.category_map = {
            'default': [80, 443, 554, 8080, 8443, 8554, 37777, 37778, 37779, 3702, 1935, 8000, 8001, 8081]
        }
        self.all_ports = self.category_map['default']
        self.priority_ports = self.category_map['default']
    ...
    def get_ports_for_scan_mode(self, mode: str) -> List[int]:
        ...
        if mode == "FAST":
            ...
            if 'standard_web' in self.category_map:
                balanced_ports.update(self.category_map['standard_web'])
            if 'rtsp_ecosystem' in self.category_map:
                balanced_ports.update(self.category_map['rtsp_ecosystem'])
            if 'streaming_protocols' in self.category_map:
                if 'rtmp' in self.category_map['streaming_protocols']:
                    balanced_ports.update(self.category_map['streaming_protocols']['rtmp'])
