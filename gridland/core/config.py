...
class GridlandConfig:
    ...
    default_ports: str = field(
        default_factory=lambda: os.getenv(
            "GL_DEFAULT_PORTS",
            "80,443,554,8080,8443,8554,37777,37778,37779,3702,1935,8000,8001,8081,10554,1554,2554,8888,9999,5000",
        )
    )
    ...
    verbose: bool = field(
        default_factory=lambda: os.getenv("GL_VERBOSE", "false").lower() == "true"
    )
    color_output: bool = field(
        default_factory=lambda: os.getenv("GL_COLOR", "true").lower() == "true"
    )

    # Authentication testing
    test_default_creds: bool = field(
        default_factory=lambda: os.getenv("GL_TEST_CREDS", "true").lower() == "true"
    )
    auth_delay: float = field(default_factory=lambda: float(os.getenv("GL_AUTH_DELAY", "1.0")))
    max_auth_attempts: int = field(
        default_factory=lambda: int(os.getenv("GL_MAX_AUTH_ATTEMPTS", "3"))
    )

    # API Keys and external services
    shodan_api_key: str | None = field(default_factory=lambda: os.getenv("GL_SHODAN_API_KEY"))
    censys_api_id: str | None = field(default_factory=lambda: os.getenv("GL_CENSYS_API_ID"))
    censys_api_secret: str | None = field(default_factory=lambda: os.getenv("GL_CENSYS_API_SECRET"))
    ...
    def load_user_config(self, config_path: Path | None = None) -> dict[str, Any]:
    ...
    def save_user_config(self, config_path: Path | None = None):
    ...
    def get_ports_list(self) -> list[int]:
        ...
        if not self.default_ports:
            return []
        try:
            return sorted(list({int(port.strip()) for port in self.default_ports.split(",")}))
    ...
    def has_censys_api(self) -> bool:
        ...
        return (
            self.censys_api_id is not None
            and len(self.censys_api_id.strip()) > 0
            and self.censys_api_secret is not None
            and len(self.censys_api_secret.strip()) > 0
        )
...
class CameraPortManager:
    ...
    def _get_priority_ports(self) -> list[int]:
        ...
        return [
            ...
            8000,
            8001,
            8081,  # Common alternates
            10554,
            1554,
            2554,  # Additional RTSP
            8888,
            9999,
            5000,  # Popular alternatives
        ]
