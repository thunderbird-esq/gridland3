    ...
    def validate_port_range(port_range: str) -> bool:
        ...
        try:
            if "-" in port_range:
                ...
                return (
                    NetworkValidator.validate_port(start_port)
                    and NetworkValidator.validate_port(end_port)
...
class IPRangeGenerator:
    ...
    @staticmethod
    def chunk_ips(
        ip_generator: Generator[str, None, None], chunk_size: int = 1000
...
class PortScanner:
    ...
    def scan_multiple_targets(self, targets: list[tuple[str, list[int]]]) -> list[ScanResult]:
        ...
        with ThreadPoolExecutor(max_workers=self.max_threads) as executor:
            ...
            future_to_target = {
                executor.submit(self.scan_ports, ip, ports): (ip, ports) for ip, ports in targets
            }
    ...
    def _grab_banner(self, sock: socket.socket, max_bytes: int = 1024) -> str | None:
    ...
    def _identify_service(self, port: int, banner: str | None = None) -> str | None:
...
class NetworkUtils:
    ...
    def parse_ports(port_spec: str) -> list[int]:
        ...
        try:
            ...
            valid_ports = sorted({p for p in ports if NetworkValidator.validate_port(p)})
            return valid_ports
