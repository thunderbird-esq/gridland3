...
class SecurityLogger:
    ...
    def scan_complete(self, target: str, scan_type: str, duration: float, results_count: int = 0):
        ...
        self.info(
            f"Completed {scan_type} scan of {target} in {duration:.2f}s ({results_count} results)"
        )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            if exc_type is None:
                ...
                self.logger.error(
                    f"Failed {self.operation} on {self.target} after {duration:.2f}s: {exc_val}"
                )
...
def get_logger(name: str = "gridland", level: int | None = None) -> SecurityLogger:
