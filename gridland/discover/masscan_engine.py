...
from ..core.network import IPRangeGenerator, NetworkValidator
...
class MasscanEngine:
    ...
    def __init__(self, config=None):
        ...
        self.camera_ports = [
            ...
            8080,
            8081,
            8000,  # HTTP alternatives
            8443,
            8888,
            9000,  # HTTPS alternatives
            81,
            82,
            83,  # HTTP alternatives
            5000,
            5001,  # Camera management
            37777,
            37778,  # Dahua
            8899,  # Hikvision
            4567,  # Axis
            1024,
            1025,  # Various camera systems
        ]

    def _find_masscan(self) -> str | None:
    ...
    def scan_range(
        self, ip_range: str, ports: list[int] | None = None, rate: int | None = None
    ) -> list[MasscanResult]:
        """
        ...
        try:
            ...
            logger.info(
                f"Starting masscan scan of {ip_range} ({len(ports)} ports, rate: {rate} pps)"
            )
    ...
    def _validate_range(self, ip_range: str) -> bool:
        ...
        if "-" in ip_range and len(ip_range.split("-")) == 2:
            ...
            return NetworkValidator.validate_ip(start_ip.strip()) and NetworkValidator.validate_ip(
                end_ip.strip()
            )
    ...
    def _build_command(
        self, ip_range: str, ports: list[int], rate: int, output_file: Path
    ) -> list[str]:
    ...
    def _fallback_scan(self, ip_range: str, ports: list[int] | None) -> list[MasscanResult]:
        ...
        ports = ports or self.camera_ports
        scanner = PortScanner(
            timeout=self.config.connect_timeout, max_threads=self.config.max_threads
        )
    ...
    def scan_targets_file(
        self, file_path: str, ports: list[int] | None = None
    ) -> list[MasscanResult]:
    ...
    def get_unique_ips(self, results: list[MasscanResult]) -> list[str]:
        ...
        return list({result.ip for result in results})
