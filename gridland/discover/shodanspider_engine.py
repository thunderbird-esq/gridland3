...
class ShodanSpiderEngine:
    ...
    def _find_shodanspider(self) -> str | None:
        ...
        logger.warning(
            "ShodanSpider v2 not found. Install from: https://github.com/shubhamrooter/ShodanSpider"
        )
        return None
    ...
    def search_cameras(
        self, query: str = "camera", limit: int = 1000, country: str | None = None
    ) -> list[ShodanSpiderResult]:
        """
    ...
    def search_camera_brands(
        self, brands: list[str], limit: int = 1000
    ) -> list[ShodanSpiderResult]:
        """
    ...
    def search_by_port(
        self, port: int, service: str = "", limit: int = 1000
    ) -> list[ShodanSpiderResult]:
        """
    ...
    def _execute_search(self, query: str, limit: int) -> list[ShodanSpiderResult]:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=120, check=False  # 2 minute timeout
            )
    ...
    def _execute_cve_search(self, cve_id: str, limit: int) -> list[ShodanSpiderResult]:
        ...
        try:
            cmd = [self.shodanspider_path, "-cve", cve_id, "-o", str(output_file)]
                            ...
                            ShodanSpiderResult(
                                ...
                                service=(
                                    ...
                                    if port in [80, 8080]
                                        ...
                                        "https"
                                        if port == 443
                                        else "rtsp" if port == 554 else "unknown"
                                    )
                                ),
                                banner="",
                                country="",
    ...
    def _parse_entry(self, entry: dict[str, Any]) -> ShodanSpiderResult | None:
        ...
        try:
            ...
            if "vulns" in entry:
                vulns = (
                    list(entry["vulns"].keys())
                    if isinstance(entry["vulns"], dict)
                    else entry["vulns"]
                )
            elif "cves" in entry:
                vulns = entry["cves"]
    ...
    def search_multiple_queries(
        self, queries: list[str], limit_per_query: int = 100
    ) -> list[ShodanSpiderResult]:
