...
║                         DEPRECATION NOTICE                                    ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  CamXploit.py has been DEPRECATED in favor of GRIDLAND v3.0                  ║
║                                                                               ║
║  GRIDLAND v3.0 provides:                                                      ║
║  • Modern modular architecture with plugin system                             ║
║  • Comprehensive test suite (521+ tests)                                      ║
║  • Enhanced CLI with rich features                                            ║
║  • Structured data files (JSON databases)                                     ║
║  • Performance improvements (~205K detections/sec)                            ║
║                                                                               ║
║  MIGRATION GUIDE:                                                             ║
║  ────────────────────────────────────────────────────────────────────────────║
║  OLD: python CamXploit.py                                                     ║
║  NEW: gridland analyze <IP> --full-scan                                       ║
║                                                                               ║
║  OLD: (port scanning in CamXploit)                                            ║
║  NEW: gridland discover --use-python-scanner --camera-ports --target <IP>     ║
║                                                                               ║
║  For full documentation, see: README.md                                       ║
║  For CLI reference, run: gridland --help                                      ║
╚══════════════════════════════════════════════════════════════════════════════╝

WARNING: This script is maintained for backward compatibility only.
         No new features will be added. Please migrate to GRIDLAND v3.0.
"""

import warnings as _warnings_module
_warnings_module.warn(
    "CamXploit.py is deprecated. Use 'gridland analyze <IP> --full-scan' instead. "
    "See README.md for migration guide.",
    DeprecationWarning,
    stacklevel=2
)

# Original CamXploit.py code continues below...
...
CVE_DATABASE = {
    ...
    "cp plus": ["CVE-2021-XXXXX", "CVE-2022-XXXXX", "CVE-2023-XXXXX"],
}
...
def print_search_urls(ip):
    ...
    print(
        f"  🔹 Google Dorking (Quick Search): https://www.google.com/search?q=site:{ip}+inurl:view/view.shtml+OR+inurl:admin.html+OR+inurl:login"
    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "loc" in data:
                ...
                print(
                    f"    🔗 Google Earth: https://earth.google.com/web/@{lat},{lon},0a,1000d,35y,0h,0t,0r"
                )
...
def validate_ip(target_ip):
    try:
        ...
        if ip.is_private:
            print(
                f"{R}[!] Warning: Private IP address detected. This tool is meant for public IPs.{W}"
            )
        return True
    except ValueError:
    ...
    def analyze_port(port):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if any(
                    x in content
                    for x in ["cp plus", "cp-plus", "cpplus", "cp_plus", "uvr", "0401e1"]
                ):
            ...
            for endpoint in endpoints:
                try:
                    ...
                    endpoint_response = requests.head(
                        endpoint_url, headers=HEADERS, timeout=TIMEOUT, verify=False
                    )
                    if endpoint_response.status_code in [200, 401, 403]:
                        print(
                            f"    ✅ Camera Endpoint Found: {endpoint_url} (HTTP {endpoint_response.status_code})"
                        )
                        camera_indicators = True
                except:
                    continue
            ...
            if response.status_code == 200:
                ...
                if "<title>" in content:
                    ...
                    if title_end > title_start:
                        ...
                        if any(
                            x in title
                            for x in ["dvr", "nvr", "recorder", "surveillance", "cctv", "camera"]
                        ):
    ...
    for port in open_ports:
        ...
        try:
            ...
            if "hikvision" in server_header:
                ...
                x in content for x in ["cp plus", "cp-plus", "cpplus", "cp_plus", "uvr", "0401e1"]
            ):
...
def check_stream(url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if any(
                x in content_type
                for x in ["video", "stream", "mpeg", "h264", "mjpeg", "rtsp", "rtmp"]
            ):
                return True
        ...
        if response.status_code == 200:
            ...
            if any(
                x in content_type
                for x in ["video", "stream", "mpeg", "h264", "mjpeg", "rtsp", "rtmp", "image"]
            ):
                return True

            # Check for video file extensions in URL
            if any(
                x in url.lower() for x in [".mp4", ".m3u8", ".ts", ".flv", ".webm", ".avi", ".mov"]
            ):
                return True
    ...
    def check_stream_with_details(url):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if any(
                    x in content_type for x in ["video", "stream", "mpeg", "h264", "mjpeg", "image"]
                ):
                    ...
                    x in url.lower()
                    for x in [".mp4", ".m3u8", ".ts", ".flv", ".webm", ".avi", ".mov"]
                ):
                    ...
                    x in url.lower() for x in ["/video", "/stream", "/live", "/mjpg", "/snapshot"]
                ):
    ...
    try:
        ...
        if open_ports:
            ...
            if not camera_found:
                choice = (
                    input(
                        "\n[❓] No camera found. Do you still want to check login pages, vulnerabilities, and passwords? [y/N]: "
                    )
                    .strip()
                    .lower()
                )
